...
def run_pdf_cleaner(path_str: str):
    """PDF 扫描件智能去黑边核心逻辑 (接入 bridge 终端版)"""
    try:
        ...
        bridge.update_terminal(f"[*] 共找到 {len(pdf_files)} 个 PDF 文件，准备处理...")
        
        success_count = 0
        for idx, pdf_path in enumerate(pdf_files):
            bridge.update_terminal(f"[*] 正在处理 ({idx + 1}/{len(pdf_files)}): {pdf_path.name}")
            ...
            try:
                ...
                for page_num in range(total_pages):
                    if page_num % 10 == 0:
                        bridge.update_terminal(f"  └─ 正在扫描页面 {page_num + 1}/{total_pages}...")
                        time.sleep(0.01)
            ...
            success_count += 1
            bridge.update_terminal(f"  └─ ✅ 保存成功: {out_path.name}")
