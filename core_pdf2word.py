# -*- coding: utf-8 -*-
import eel, tempfile, fitz, os, threading, time
...
def run_pdf2word(path_str, mode, dpi):
    try:
        ...
        if p.is_file():
            ...
            eel.update_terminal(f"[*] 检测到文件夹，正在扫描 PDF 文档: {p.name}")
        ...
        for idx, f in enumerate(valid_files, 1):
            ...
            eel.update_terminal(f"[*] 正在转换 ({idx}/{len(valid_files)}): {f.name}")
            ...
            # 3. 将高负载任务挂入独立线程
            if mode == "editable":
                eel.update_terminal(f"  └─ 正在深度分析排版 (若文件超大可能需要几分钟，请耐心等待)...")
