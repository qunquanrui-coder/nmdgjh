...
def ocr_image_file(image_path: str) -> str:
    ...
    try:
        ...
        if result: return " ".join([line[1] for line in result])
...
def extract_text_from_pdf(file_path: str) -> str:
    ...
    full_text = "\n".join(text_parts)
    if len(clean_text(full_text)) >= 30: return full_text

    if ENGINE is None: return full_text
    try:
        with fitz.open(file_path) as doc:
            ...
            for i in range(min(doc.page_count, 2)):
                ...
                with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp: temp_img = tmp.name
                try:
                    pix.save(temp_img)
                    ocr_parts.append(ocr_image_file(temp_img))
                finally:
    ...
    except Exception: return full_text
...
def run_invoice(path, recursive):
    try:
        ...
        for idx, f in enumerate(valid, 1):
            eel.update_terminal(f"[*] 正在处理 ({idx}/{len(valid)}): {f.name}")
            ext = f.suffix.lower().lstrip(".")
            full_text = extract_text_from_pdf(str(f)) if ext == "pdf" else ocr_image_file(str(f))
